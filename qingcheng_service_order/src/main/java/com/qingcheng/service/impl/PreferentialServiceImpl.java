...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public int findPreMoneyByCategoryId(Integer categoryId, int money) {
        ...
        criteria.andEqualTo("state","1");//状态正常
        criteria.andEqualTo("categoryId",categoryId);
        criteria.andLessThanOrEqualTo("buyMoney",money);// 小于等于优惠金额
        criteria.andLessThanOrEqualTo("startTime",new Date());// 活动开始时间小于当前时间
        criteria.andGreaterThanOrEqualTo("endTime",new Date());// 结束时间大于当前时间
        example.setOrderByClause("buy_money desc"); //根据购买金额倒叙排序
        ...
        if (preferentialList.size() > 0){
            ...
            if ("1".equals(preferential.getType())){//不翻倍
                Integer preMoney = preferential.getPreMoney();
                return preMoney;
            }else {
                Integer preMoney = preferential.getPreMoney() * (money / preferential.getBuyMoney());
                return preMoney;
            }
        }
        return 0;
