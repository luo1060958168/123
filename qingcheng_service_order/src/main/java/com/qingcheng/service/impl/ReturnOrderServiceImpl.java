...
public class ReturnOrderServiceImpl implements ReturnOrderService {
     ...
     */
    @Override
    public void rejectRefund(String id, String remark, Integer admin) {
        ...
        returnOrder.setAdminId(admin);// 处理人
        returnOrder.setRemark(remark);// 驳回消息
        returnOrder.setApplyTime(new Date());// 处理时间
        returnOrderMapper.updateByPrimaryKeySelective(returnOrder);// 保存

        // 修改对应订单明细的退款状态为未申请
        Example example = new Example(ReturnOrderItem.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("returnOrderId",id);// 退货订单id
        ...
        for (ReturnOrderItem returnOrderItem : returnOrderItems) {
            ...
            Long orderItemId = returnOrderItem.getOrderItemId();// 提取订单明细id
            orderItem.setId(orderItemId+"");
            orderItem.setIsReturn("0");
            orderItemMapper.updateByPrimaryKeySelective(orderItem);// 更新状态让其可以重新申请退款
        }
    }
     ...
     */
    @Override
    public void agreeRefund(String id, Integer money, Integer admin) {
        ReturnOrder returnOrder = returnOrderMapper.selectByPrimaryKey(id);
        if (returnOrder == null){
            throw new RuntimeException("退款订单不存在");
        }
        ...
        returnOrder.setAdminId(admin);// 操作人
        returnOrder.setReturnMoney(money);// 退款金额
        returnOrder.setApplyTime(new Date());// 处理时间
        returnOrderMapper.updateByPrimaryKeySelective(returnOrder);// 保存操作
