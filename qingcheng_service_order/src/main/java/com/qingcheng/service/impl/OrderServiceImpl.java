...
public class OrderServiceImpl implements OrderService {
    ...
    public void orderTimeOutLogic() {
        ...
        Integer orderTimeout = orderConfig.getOrderTimeout(); // 超时时间分钟 60分
    ...
    public void updatePayStatus(String orderId, String transactionId) {
        ...
        if (order != null && "0".equals(order.getPayStatus())){// 订单存在且订单状态为0
    ...
    public void batchSend(List<Order> orders) {
        // 判断快递公司及订单是否为空
        for (Order order : orders) {
            if (order.getShippingCode()==null || order.getShippingName() == null || "".equals(order.getShippingCode())){
                throw new RuntimeException("请选择快递公司和填写快递订单号");
            }
        }
    ...
    public Map<String, Object> add(Order order) {
        ...
        // 获取选中的购物车
        List<OrderItem> orderItemList = newOrderItemList.stream().filter(cart -> (boolean) cart.get("checked")).map(cart -> (OrderItem) cart.get("item")).collect(Collectors.toList());
        // 扣减库存
        if (!skuService.deductionStock(orderItemList)){
            new RuntimeException("库存扣减失败");
        }
        // 保存订单主表
        ...
        try {
            ...
            double proportion = (double)order.getPayMoney()/totalMoney; // 折扣
            // 保存订单明细
            for (OrderItem orderItem : orderItemList) {
                orderItem.setOrderId(order.getId());// 订单主表
...
//            int num = 1/0;
    ...
    private Example createExample(Map<String, Object> searchMap){
        ...
        if(searchMap!=null){
            ...
            //根据id数组查询
            if (searchMap.get("ids") != null && !"".equals(searchMap.get("ids"))){
...
//                System.out.println((Iterable) searchMap.get("ids"));
            }
        }
        return example;
    }
