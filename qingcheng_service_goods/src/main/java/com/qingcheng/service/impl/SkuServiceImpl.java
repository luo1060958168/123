...
public class SkuServiceImpl implements SkuService {
    ...
    public void importToEs() {
        ...
        RestHighLevelClient restHighLevelClient = new RestHighLevelClient(builder);// 高级客户端连接对象

        if (countEs(restHighLevelClient)>0){
            System.out.println("已存在索引数据库");
            return;
        }
...
//        BoundHashOperations boundHashOperations = redisTemplate.boundHashOps(CacheKey.SKU_PRICE);
//        if (boundHashOperations == null){
//            redisTemplate.boundHashOps(CacheKey.SKU_PRICE).put(id,0);
//        }
        Integer price = (Integer) redisTemplate.boundHashOps(CacheKey.SKU_PRICE).get(id);
        if (price == null){
            price = skuMapper.selectByPrimaryKey(id).getPrice();
    ...
    public boolean deductionStock(List<OrderItem> orderItemList) {
        ...
        for (OrderItem orderItem : orderItemList) {
            ...
            if (!"1".equals(sku.getStatus())){// 状态非正常
                isDeduction = false;
                break;
            }
            if (orderItem.getNum().intValue()>sku.getNum().intValue()){// 购买件数大于 库存
                isDeduction = false;
                break;
            }
     ...
     * @param skuList
     */
    public void importSkuListToEs(RestHighLevelClient restHighLevelClient, List<Sku> skuList){
        // 构建BulkRequest
